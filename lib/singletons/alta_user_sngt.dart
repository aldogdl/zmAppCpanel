...
import 'package:zmcpanel/entity/perfil_entity.dart';
...
class AltaUserSngt {
  ...
  bool isAtras = false;
  String _movilTel;
  String get movilTel => this._movilTel;
  void setMovilTel(String movil) => this._movilTel = movil;

  List<String> _rolesFromPagWeb;
  List<String> get rolesFromPagWeb => this._rolesFromPagWeb;
  void setRolesFromPagWeb(List roles) {
    this._rolesFromPagWeb = new List<String>.from(roles);
  }
  ...
  void setCreateDataSitioWebByCarruselByKey(String key, String subKey, dynamic valor) {
    ...
    if(!this.createDataSitioWeb['carrucel'].containsKey(key)){
      this.createDataSitioWeb['carrucel'].putIfAbsent(key, () => {subKey: valor});
    }else{
       if(!this.createDataSitioWeb['carrucel'][key].containsKey(subKey)){
        this.createDataSitioWeb['carrucel'][key].putIfAbsent(subKey, () => valor);
      }else{
        print(this.createDataSitioWeb['carrucel']);
  ...
  void dispose() {
    ...
    this.createDataSitioWeb = _getDataSitioEmpty();
    this._rolesFromPagWeb = new List();
    this._movilTel = null;
  }
}